...
const emitBackupEvent = (event, payload) => {
    try {
        ...
        // Socket.io no inicializado aún (ej: durante el arranque antes del servidor HTTP)
        console.warn(`[Backup] No se pudo emitir evento "${event}" — Socket.IO no disponible`);
    }
};

/**
 * @param {Object} outputStream - Stream de respuesta (res) para descarga directa por HTTP.
 * @param {Object} config - Objeto de configuración que contiene backupFrequencyDays y lastBackupDate.
 */
    ...
    try {
        ...
        if (outputStream) {
            ...
            await new Promise((resolve, reject) => {
                ...
                // 'close' es a prueba de balas: se dispara cuando Express termina 
                // de enviar el archivo, o si el usuario cancela la descarga a la mitad.
        ...
        console.log(`[Backup] Respaldo completado en ${(durationMs / 1000).toFixed(1)}s`);
